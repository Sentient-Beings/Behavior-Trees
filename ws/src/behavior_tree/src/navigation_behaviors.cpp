// ALL the behaviors for navigation
...
BT::NodeStatus SetLocations::tick() {
    ...
    try {
        ...
        // Shuffle location names to get a random order in each run
...
GetLocationFromQueue::GetLocationFromQueue(const std::string &name, const BT::NodeConfig &config) :
    BT::SyncActionNode(name, config) {
        std::cout << "[" << this->name() << "] Initialized" << std::endl;
    }
...
BT::NodeStatus GetLocationFromQueue::tick() {
    ...
    if (location_queue_.value().empty()) {
        ...
        // Set the current target location as an output
